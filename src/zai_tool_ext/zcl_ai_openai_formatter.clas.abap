CLASS zcl_ai_openai_formatter DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_s_format,
        type     TYPE string,
        function TYPE ty_s_format_function,
      END OF ty_s_format.
...
CLASS zcl_ai_openai_formatter IMPLEMENTATION.
  METHOD zif_ai_tool_formatter~format.
    ...
    <fs_format> = VALUE #( type     = 'function'
                           function = VALUE #( name        = iv_name
                                               description = iv_description
                                               parameters  = is_schema ) ).
    <fs_format>-function-parameters-required = VALUE #( FOR req IN is_schema-required
                                                        ( to_lower( req ) ) ).
  ENDMETHOD.
